...
file = "data"
df = pd.read_csv(file, sep=",", header=None, names=['Without Gzip','With Gzip'])


print ("CIs Without Gzip: ", mean_confidence_interval(df['Without Gzip']))
print ("CIs With Gzip", mean_confidence_interval(df['With Gzip']))
...
CI_Chrome = mean_confidence_interval(df['Without Gzip'])
CI_Explorer = mean_confidence_interval(df['With Gzip'])
...
df_CI = pd.DataFrame(list(zip(CI_Chrome, CI_Explorer)), columns = ['Test 1','Test 2'])
...
#plt.xlabel('Browsers')
plt.title('Standard Error Diagram')
plt.show()
